...
describe('resolveTabLabels', () => {
  ...
  it('resolves dynamic labels with counts', () => {
    ...
    // Project 'weeks' tab has a static label
    const weeksTab = resolved.find(t => t.id === 'weeks');
    expect(weeksTab?.label).toBe('Weeks');

    // Program 'weeks' tab has a dynamic label
    const programTabs = getTabsForDocumentType('program');
    const resolvedProgram = resolveTabLabels(programTabs, { ...mockDocument, document_type: 'program' }, { weeks: 3 });
    const programWeeksTab = resolvedProgram.find(t => t.id === 'weeks');
    expect(programWeeksTab?.label).toBe('Weeks (3)');
  });
