    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-2.5">
          ...
          {week.is_current && (
            <span className="text-xs bg-accent/10 text-accent-foreground px-1.5 py-0.5 rounded">Current</span>
          )}
        </div>
            ...
            {standups.map((slot: StandupSlot) => {
              ...
              const dateLabel = (
                <div className="w-20 flex-shrink-0">
                  <span className={cn('text-xs font-medium', isToday ? 'text-accent-foreground' : 'text-muted')}>
                    {slot.day.slice(0, 3)}
                  </span>
